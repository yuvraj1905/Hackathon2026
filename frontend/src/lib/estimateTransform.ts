...
function normalizeTechStackFromAgent(
    ...
    if (
        d.backend &&
        typeof d.backend === "object" &&
        !Array.isArray(d.backend)
    ) {
    ...
    if (
        ...
        const buildName = getName(
            inf.mobile_deployment && typeof inf.mobile_deployment === "object"
                ? (inf.mobile_deployment as Record<string, unknown>)
                      .build_service
                : null,
        );
...
function buildTechStackRecs(techStackData: any): TechRec[] {
    ...
    return [
        ...(Array.isArray(techStackData.frontend) &&
        techStackData.frontend.length
            ? [
                  {
                      category: "Frontend",
        ...
        ...(Array.isArray(techStackData.backend) && techStackData.backend.length
            ...
            : []),
        ...(Array.isArray(techStackData.database) &&
        techStackData.database.length
            ? [
                  {
                      category: "Database",
...
export function transformEstimateResponse(data: any): TransformEstimateResult {
    ...
    const tasks: SubTask[] = features.map((f: any, i: number) => {
        // API sends total_hours per feature (estimated_hours is legacy)
        ...
        let children: SubTask[] | undefined =
            subfeatures.length > 0
                ? subfeatures.map((sf: any, j: number) => {
                      ...
                      return {
                          id: `${i + 1}-${j + 1}`,
                          name: (sf.name || "").trim() || `Subfeature ${j + 1}`,
                          frontend: s.fe,
                          backend: s.be,
                          integration: s.integ,
                          testing: s.test,
    ...
    const rawDomain = (domainDetection.detected_domain ?? "")
        .toString()
        .replace(/_/g, " ")
        .trim()
        .toLowerCase();
    const domainStr =
        rawDomain && rawDomain !== "unknown" ? rawDomain : "";
    const moduleName = domainStr
        ? domainStr.charAt(0).toUpperCase() + domainStr.slice(1)
        : (data.metadata?.build_options?.[0]
            ? String(data.metadata.build_options[0]).charAt(0).toUpperCase() +
              String(data.metadata.build_options[0]).slice(1).toLowerCase()
            : "Project");
    ...
    const platforms = frontendKeys.map(
    ...
    return {
        ...
        techStackStructured:
            techStackData && typeof techStackData === "object"
                ? (techStackData as Record<string, unknown>)
                : null,
        resources,
        summary,
        proposal,
