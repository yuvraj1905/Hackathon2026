...
export async function estimateProject(
  ...
  try {
    ...
    if (payload.file) {
      ...
      if (payload.preferred_tech_stack?.length) {
        formData.append("preferred_tech_stack", payload.preferred_tech_stack.join(","));
      }

      response = await fetch(`${API_BASE_URL}/estimate`, {
        method: "POST",
        body: formData,
      });
    } else {
